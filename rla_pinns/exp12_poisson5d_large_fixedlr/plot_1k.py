...
project = "exp12_poisson5d_large_fixedlr"  # name from the 'Projects' tab on wandb
...
sweep_ids = {  # ids from the wandb agent
    ...
    "ec21dqc6": "ENGD (Woodbury)",
    "fsxpdoms": "ENGD (Nystrom)",
    "gto5ep5k": "SPRING",
    "x9u551s6": "SPRING (Nystrom)",
}
...
if __name__ == "__main__":
    ...
    for (x, xlabel), (y, ylabel) in product(x_to_xlabel.items(), y_to_ylabel.items()):
        with plt.rc_context(
            ...
            ax.legend()
            plt.savefig(path.join(HEREDIR, f"{y}_over_{x}_1k.pdf"), bbox_inches="tight")
